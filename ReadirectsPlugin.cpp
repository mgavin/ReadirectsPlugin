...
const int MAGICPHYSICSTICKSPERSEC = 119;
...
void ReadirectsPlugin::onLoad() {
	...
	cvarManager->registerCvar("readirects_enabled", "0", "Enable ReadirectsPlugin", true, true, 0, true, 1)
		.addOnValueChanged([this](std::string oldValue, CVarWrapper cvar) {
			pluginIsEnabled = cvar.getBoolValue();
			if (pluginIsEnabled && !gameHooked) {
				HookGameEngine();
			} else {
				UnhookGameEngine();
			}
	...
	cvarManager
		->registerCvar("readirects_timer_seconds", "1", "Seconds between redirecting the ball", false, true, 1, true, 60)
		.addOnValueChanged(
			[this](std::string oldVal, CVarWrapper cvar) { launchBallTimer = MAGICPHYSICSTICKSPERSEC * cvar.getIntValue(); });
	cvarManager
		->registerCvar("readirects_numballhitscar",
									 "0",
									 ...
									 true,
									 10)
		.addOnValueChanged([this](std::string oldVal, CVarWrapper cvar) { numCarHitsBall = cvar.getIntValue(); });
	cvarManager
		->registerCvar("readirects_numballhitsground",
									 "0",
									 ...
									 true,
									 25)
		.addOnValueChanged([this](std::string oldVal, CVarWrapper cvar) { numGroundHitsBall = cvar.getIntValue(); });
...
std::string ReadirectsPlugin::next_in_playlist() {
	if (playlist.empty())
		return "";
	if (playlist_cpy.empty())
		std::copy(begin(playlist), end(playlist), std::back_inserter(playlist_cpy));

	CVarWrapper random_playlist = cvarManager->getCvar("readirects_enable_randomizeplaylist");
	std::string ret;
	// FIXME, invalidation of iterator possibly
	if (!random_playlist.getBoolValue()) {
		ret = playlist_cpy.front();
		playlist_cpy.pop_front();
	} else {
		std::size_t r	 = random(0, playlist_cpy.size() - 1);
		auto				it = begin(playlist_cpy) + r;
		ret						 = *it;
		playlist_cpy.erase(it);
	}
...
void ReadirectsPlugin::LaunchBall(std::vector<std::string> params) {
	//  main driver
	if (!pluginIsEnabled && !gameWrapper->IsInFreeplay())
		return;
...
void ReadirectsPlugin::OnBallHitsGround(std::string eventName) {
	...
	if (numGroundHitsBall <= 0) {
		numGroundHitsBall = cvarManager->getCvar("readirects_numballhitsground").getIntValue();
		LaunchBall({""});
	} else {
		numGroundHitsBall--;
	}
...
void ReadirectsPlugin::OnGameTick(std::string eventName) {
  ...
  // so ((number of seconds) x 120) == each tick;
	if (launchBallTimer <= 0) {
		...
		//     lastTimePoint).count()));
		launchBallTimer = cvarManager->getCvar("readirects_timer_seconds").getIntValue() * MAGICPHYSICSTICKSPERSEC;
		lastTimePoint		= std::chrono::system_clock::now();
